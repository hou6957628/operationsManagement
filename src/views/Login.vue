        ...
        <div class="contentBox">
            ...
            <div class="content">
                <el-form :model="ruleForm" :rules="rules" ref="ruleForm" class="demo-ruleForm">
                    ...
                    <el-form-item prop="password" style="margin-bottom: 20px;font-size: 16px;">
                    ...
                    <el-form-item prop="captcha" style="font-size: 16px;">
                        <el-input class="captchaStyle" type="text" v-model="ruleForm.captcha"></el-input>
                        <!--<img :src="captchaImg" class="captchaImg" @click="getCaptcha">-->
                        <img :src="captchaImg" class="captchaImg" @click="getCaptcha">
                    </el-form-item>
                    <el-form-item style="text-align: left;margin-bottom: 0;">
                        <el-checkbox-group v-model="ruleForm.type">
                            <el-link :underline="false" @click="findPassword" class="findPassword">{{$t("button.changePassword")}}</el-link>
                        </el-checkbox-group>
                    </el-form-item>
                    <el-button class="btn" type="primary" @click="submitForm('ruleForm')">{{$t('button.login')}}</el-button>
                </el-form>
            </div>
        </div>
        <el-dialog class="el_dia" title="修改密码" :visible.sync="dialogFormVisible">
            <el-form :model="rulePasswordForm" :rules="rulePassword" ref="rulePasswordForm" >
                <el-form-item label="账户" prop="account" :label-width="formLabelWidth">
                    <el-input v-model="rulePasswordForm.account" autocomplete="off"></el-input>
                </el-form-item>
        ...
        data() {
            ...
            return {
                ...
                rules: {
                    ...
                    captcha: [
                        {required: true, message: '请输入图形', trigger: 'blur'},
                        {min: 5, max: 5, message: '图型验证码格式不对', trigger: 'blur'}
                    ],
                },
                rulePassword: {
                    account: [
                ...
                formLabelWidth: '80px',
                captchaImg:'http://127.0.0.1:8080/apis/admin/auth/getCaptcha',
            };
        },
        methods: {
            //登陆
                    ...
                    if (valid) {
                        this.$Api.Login.loginIn({
                            ...
                            if (res.result == '00000000') {
                                ...
                                //把loginToken放入cookie中
                                this.$cookie.set("loginToken", res.data.loginToken, {
                                    expires: 1
                                });
                                sessionStorage.setItem('getNaviMenuList', JSON.stringify(res.data.naviMenuList[0].children));
                                sessionStorage.setItem('actionList', JSON.stringify(res.data.user.authList));
                                //设置用户名,手机号
                                this.$store.commit("getUserName", res.data.user.userFullName);
                                this.$store.commit("getMobile", res.data.user.mobile);
                                sessionStorage.setItem('userFullName', JSON.stringify(res.data.user.userFullName));
                                sessionStorage.setItem('mobile', JSON.stringify(res.data.user.mobile));
            ...
            //获取图片验证码
            getCaptcha(){
                this.captchaImg="http://127.0.0.1:8080/apis/admin/auth/getCaptcha?t="+new Date().getTime();
            },
            //找回密码
                ...
                this.$refs[formName].validate((valid) => {
                    ...
                    if (valid) {
                        this.$Api.Login.fingPassword({
                            mobile:this.rulePasswordForm.account,
            ...
            getLanguageType(){
                ...
                console.log("language"+this.$cookie.get('language'))
            }
        },
        mounted() {
